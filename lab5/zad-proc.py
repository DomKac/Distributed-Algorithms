...
class Node(Process):
    ...
    def mine_block(self):
        ...
        with self.lock:
            new_block = Block(
                index=(self.network[self.node_id][-1].index + 1),
                transactions=next_trans,
                previous_hash=self.network[self.node_id][-1].hash,
            )
    ...
    def resolve_conflicts_2(self):
        ...
        with self.lock:
            ...
            # longest_chain = max(self.network.values(), key=lambda chain: chain[-1].index if chain else -1)
            ...
            if len(longest_chain) > len(self.network[self.node_id]):
                ...
                # trzeba opdowiednio usunąć transakcje z puli
                self.transactions_pool = self.transactions_pool[len(longest_chain) - len(self.network[self.node_id]):]
    ...
    def run(self):
        ...
        while self.transactions_pool:   # kończymy działanie węzła, gdy nie ma już transakcji w puli
            ...
            if mined_block:
                ...
                self.broadcast_block_2(mined_block)

            self.resolve_conflicts_2()
            # time.sleep(1)
        print(f"{Fore.BLUE}Node {self.node_id} finished mining{Style.RESET_ALL}")
        with self.lock:
            print(f"Node {self.node_id} chain: {Fore.YELLOW}{self.network[self.node_id]}{Style.RESET_ALL}")
